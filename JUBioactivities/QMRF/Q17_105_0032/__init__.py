...
def _read_d(file, raw=False):
    df = utils.read_labelled_molecules(file, key='Measured', name='AquSol_Q17_105_0032')
    if raw:
        return df
...
def _read_s(file, raw=False):
    df = utils.read_labelled_molecules(file, key='Measured', name='LogKmHL_Q17_66_0019', structures=True)
    if raw:
        return df
    df = utils.drop_rows(df)
