...
def _read_d(file, raw=False):
    df = utils.read_labelled_molecules(file, key='LogMolar', name='LogMolar_Q17_13_0012')
    if raw:
        return df
...
def _read_s(file, raw=False):
    df = utils.read_labelled_molecules(file, key='LogMolar', name='LogMolar', structures=True)
    if raw:
        return df
    df = utils.drop_rows(df)
