...
def _read_d(file, raw=False):
    df = utils.read_labelled_molecules(file, key='Ready_Biodeg', name='Ready_Biodeg_Q17_23a_0014')
    if raw:
        return df
...
def _read_s(file, raw=False):
    df = utils.read_labelled_molecules(file, key='Ready_Biodeg', name='Ready_Biodeg', structures=True)
    if raw:
        return df
    df = utils.drop_rows(df)
