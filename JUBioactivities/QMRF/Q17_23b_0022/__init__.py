...
def _read_d(file, raw=False):
    df = utils.read_labelled_molecules(file, key='LogHalfLife', name='LogHalfLife_Q17_23b_0022')
    if raw:
        return df
...
def _read_s(file, raw=False):
    df = utils.read_labelled_molecules(file, key='LogHalfLife', name='LogHalfLife', structures=True)
    if raw:
        return df
    df = utils.drop_rows(df)
