...
def _read_d(file, raw=False):
    df = utils.read_labelled_molecules(file, key='log M-1-LC50', name='LogLC50_Q17_33_0032')
    if raw:
        return df
...
def _read_s(file, raw=False):
    df = utils.read_labelled_molecules(file, key='log M-1-LC50', name='LogLC50_Q17_33_0032', structures=True)
    if raw:
        return df
    df = utils.drop_rows(df)
