...
def _read_d(file, raw=False):
    df = utils.read_labelled_molecules(file, key='iscarc', name='Carc_Q17_412_0052')
    if raw:
        return df
...
def _read_s(file, raw=False):
    df = utils.read_labelled_molecules(file, key='iscarc', name='Carc_Q17_412_0052', structures=True)
    if raw:
        return df
    df = utils.drop_rows(df)
