...
def _read_d(file, raw=False):
    df = utils.read_labelled_molecules(file, key='toxic', name='DevTox_Q17_416_0041')
    if raw:
        return df
...
def _read_s(file, raw=False):
    df = utils.read_labelled_molecules(file, key='iscarc', name='DevTox_Q17_416_0041', structures=True)
    if raw:
        return df
    df = utils.drop_rows(df)
