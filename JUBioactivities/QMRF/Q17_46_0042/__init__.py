...
def _read_d(file, raw=False):
    df = utils.read_labelled_molecules(file, key='isSensitizer', name='SkinSens_Q17_46_0042')
    if raw:
        return df
...
def _read_s(file, raw=False):
    df = utils.read_labelled_molecules(file, key='isSensitizer', name='SkinSens_Q17_46_0042', structures=True)
    if raw:
        return df
    df = utils.drop_rows(df)
