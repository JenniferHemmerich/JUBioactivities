...
def read_pLC50R(raw=False):
    dat = pd.read_csv(__data_src__[0], index_col=0)
    prop = pd.read_csv(__data_src__[2], index_col=0, sep="\t")
    prop.columns = ['pLC50Rat_pLC50_Bhhatarai']
...
def read_data(raw=False):
    ...
    df = pd.concat([read_pLC50M(raw),read_pLC50R(raw)],axis=1)
    return df
