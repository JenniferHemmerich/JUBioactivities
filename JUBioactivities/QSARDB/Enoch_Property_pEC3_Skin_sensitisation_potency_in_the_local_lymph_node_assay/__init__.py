...
__data_src__ += [os.path.join(__path__[0], "properties/pEC3.txt")]


def read_data(raw=False):

    df = pd.DataFrame({'pEC3_Enoch': np.loadtxt(__data_src__[-1],usecols=1, skiprows =1, delimiter=',')}, index=__data_src__[:-1])
