...
def read_carc(raw=False):
    ...
    prop = utils.convert_to_classes(prop,'C', 'nC')
    prop.columns = ['Carcinogenicity_Franke']
...
def read_pot(raw=False):
    dat = pd.read_csv(__data_src__[0], index_col=0)
    prop = pd.read_csv(__data_src__[2], index_col=0,sep="\t")
    prop.columns = ['Carc_Potency_Franke']
...
def read_data(raw=False):
    ...
    df = pd.concat([read_pot(raw),read_carc(raw)],axis=1)
    return df
