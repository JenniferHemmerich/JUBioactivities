...
def read_data(raw=False):
    ...
    props = __data_src__[-15:]

    df = pd.concat([pd.read_csv(f, index_col=0, sep="\t") for f in props], axis=1)
    df = pd.concat([pd.DataFrame({'Mols': __data_src__[:-15]}), df], axis=1)
    df = df.set_index(['Mols'])
    df = df.rename(columns=lambda x: '_'.join([x,"Liu"]))
...
def read_structures(raw=False):
    df = pd.DataFrame(index=pd.Index(__data_src__[:-15]))
