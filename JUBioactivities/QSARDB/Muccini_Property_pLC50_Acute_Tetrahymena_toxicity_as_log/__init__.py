...
def read_pIGC50_neutr(raw=False):
    dat = pd.read_csv(__data_src__[0], index_col=0)
    prop = pd.read_csv(__data_src__[2], index_col=0, sep="\t")
    prop.columns = ['pIGC50_neutr_Muccini']
...
def read_pLC50_neutr(raw=False):
    dat = pd.read_csv(__data_src__[0], index_col=0)
    prop = pd.read_csv(__data_src__[3], index_col=0, sep="\t")
    prop.columns = ['pLC50_neutr_Muccini']
...
def read_data(raw=False):
    ...
    df = pd.concat([read_pIGC50(raw), read_pIGC50_neutr(raw),read_pLC50(raw),read_pLC50_neutr(raw)],axis=1)
    return df
