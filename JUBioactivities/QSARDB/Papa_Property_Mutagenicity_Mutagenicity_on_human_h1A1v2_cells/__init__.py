...
__data_src__ += [os.path.join(__path__[0], "properties/Mutagenicity.txt")]


def read_data(raw=False):
    smiles = [np.loadtxt(f, dtype='str').item() for f in __data_src__[:-1]]
    df = pd.DataFrame({'Mutagenicity_Papa': np.loadtxt(__data_src__[-1], usecols=1, delimiter='\t', skiprows=1, dtype='str')})
    ...
    df = utils.convert_to_classes(df, 'yes', 'no')
