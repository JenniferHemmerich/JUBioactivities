...
__data_src__ += [os.path.join(__path__[0], "properties/PBT_Index.txt")]


def read_data(raw=False):

    df = pd.DataFrame({'PBT_Index_Papa': np.loadtxt(__data_src__[-1], usecols=1, skiprows=1, delimiter='\t')},
                      index=np.loadtxt(__data_src__[-1], usecols=0, skiprows=1, delimiter='\t'))
