...
__data_src__ += [os.path.join(__path__[0], "properties/LogBCF.txt")]


def read_data(raw=False):
    ind = __data_src__[:-1]
    df = pd.DataFrame({'LogBCF_Fish_Piir': np.loadtxt(__data_src__[-1], usecols=1, skiprows=1, delimiter=',')}, index=ind)
