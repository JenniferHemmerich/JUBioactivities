...
def read_moa(raw=False):
    ...
    df = utils.one_hot_encode(df,cols=['MOA_moa_Schultz'])


    return df


def read_pIGCC50(raw=False):
    dat = pd.read_csv(__data_src__[0], index_col=0)
    prop = pd.read_csv(__data_src__[2], index_col=0, sep="\t")
    prop.columns = ['pIGCC50_moa_Schultz']
...
def read_LC50(raw=False):
    dat = pd.read_csv(__data_src__[0], index_col=0)
    prop = pd.read_csv(__data_src__[3], index_col=0, sep="\t")
    prop.columns = ['pLCC50_moa_Schultz']
...
def read_data(raw=False):
    ...
    df = pd.concat([read_moa(raw), read_LC50(raw), read_pIGCC50(raw)],axis=1)
    return df
