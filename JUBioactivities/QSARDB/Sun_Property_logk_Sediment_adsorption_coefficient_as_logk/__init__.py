...
__data_src__ = list(sorted(glob.glob(os.path.join(__path__[0], "compounds/[DMT]*.txt"))))
__data_src__ += [os.path.join(__path__[0], "properties/logk.txt")]


def read_data(raw=False):
    smiles = [np.loadtxt(f, dtype='str').item() for f in __data_src__[:-1]]
    df = pd.DataFrame({'logk_Sun': np.loadtxt(__data_src__[-1], usecols=1, skiprows=1, delimiter='\t')})
