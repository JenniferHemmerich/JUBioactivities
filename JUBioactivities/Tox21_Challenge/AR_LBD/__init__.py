...
def _read_d(file, raw=False):
    df = utils.read_labelled_molecules(file, key='NR-AR-LBD', name='NR_AR_LBD_Tox21')
    if raw:
        return df
...
def _read_s(file, raw=False):
    df = utils.read_labelled_molecules(file, key='NR-AR-LBD', name='NR_AR_LBD_Tox21', structures=True)
    if raw:
        return df
    df = utils.drop_rows(df)
