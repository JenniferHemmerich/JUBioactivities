...
def _read_d(file, raw=False):
    df = utils.read_labelled_molecules(file, key='Active', name='aromatase_Tox21')
    if raw:
        return df
...
def _read_s(file, raw=False):
    df = utils.read_labelled_molecules(file, key='Active', name='aromatase_Tox21', structures=True)
    if raw:
        return df
    df = utils.drop_rows(df)
